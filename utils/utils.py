...
DATASET_KEYS = {
    ...
    'MATH_train_test_split': {'question': 'problem', 'answer': 'solution'},
    ...
    'math-verification': {'question': 'content_to_verify', 'answer': 'verification'}
}

RESPONSE_EXTRACTOR = {
    'gsm8k': lambda x: extract_answer(x, data_name='gsm8k'),
    'competition_math': lambda x: extract_answer(x, data_name='math'),
    'MATH_train_test_split': lambda x: extract_answer(x, data_name='math'),
    ...
    'math-verification': lambda x: extract_answer(x, data_name='math')
}

RESPONSE_COMPARATOR = {
    'gsm8k': lambda x, y: math_equal(x, y, timeout=True),
    'competition_math': lambda x, y: math_equal(x, y, timeout=True),
    'MATH_train_test_split': lambda x, y: math_equal(x, y, timeout=True),
    ...
    'math-verification': lambda x, y: math_equal(x, y, timeout=True)
}
...
def extract_model_shortname(model_path):
    ...
    for i in range(len(parts)-1, -1, -1):
        ...
        return part
    print(f'model short name: {parts[-1]}')
    return parts[-1]